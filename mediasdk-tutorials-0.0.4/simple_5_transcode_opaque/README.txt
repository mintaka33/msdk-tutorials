1. Introducation
    ...
    But uses the Intel Media SDK opaque memory feature. The opaque memory type hides surface allocation specifics and allows the Intel Media SDK to select the best type for the execution in hardware or software.

2. To run the example, use the following command:
    On Windows:
    ~\_build\x64\Debug>simple_transcode_opaque.exe -b 20000 -f 30/1 input.h264 output.h264
    Frame number: 100
    Execution time: 0.68 s (146.38 fps)

    On Linux:
    $ ./simple_transcode_opaque -b 20000 -f 30/1 input.h264 output.h264
    ...
    Frame number: 100
    Execution time: 0.44 s (228.82 fps)
...
3. Design
    This example shows the minimum API usage to transcodes (decode+encode) H.264 stream to another H.264 stream. It uses the system memory with opaque memory feature to allocate the frame memory,
    ...
    3) Configure the decoder video parameters to H.264 and opeque memory. 
    ...
    6) After creating the mfxExtOpaqueSurfaceAlloc, allocate the surfaces(video frame working memory) with the opaque memory.
